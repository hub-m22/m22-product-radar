...
OUT_OF_SCOPE_HEADS = ("видеопроектор", "проектор", "радиоприемник", "радиоприёмник", "кейсы и кофры", "рэковые", "конференц-систем", "кнопка вызова", "хост-ресивер", "часы ", "пейджер", "система вызова", "система беспроводных вызовов",
                      "кнопка экстренного", "система контроля доступа", "портативное радио", "экранный дисплей", "pos-", "антенна для пейджер")


OUT_OF_SCOPE_ANY = re.compile(r"кнопк\w* вызова|пейджерн|часы[- ]пейджер|аудиоконференц|конференц[- ]?систем|видеоконференц|лазерн\w+ указк|проектор|видеостен|led[- ]экран|светодиодн\w+ экран|микшерн|рэков|кофр|"
                              r"оповещени\w* о входе|комплект оповещения|при[её]мник сигнала|датчик движения|\bfm[- ]радио|\bam\s?fm\b|вызов гостя|оповещения клиентов|звуковое оборудование|аудиооборудование", re.I)
